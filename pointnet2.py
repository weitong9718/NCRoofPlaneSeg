...
class PointNet2(nn.Module):
    ...
    def forward(self, batch_dict):
        # -----------------------------
        # 1. 取坐标和输入特征
        # xyz: (B, N, 3) 只给 FPS / grouping 用
        # points: (B, N, C) 给网络做输入特征
        # -----------------------------
        if 'xyz' in batch_dict:
            xyz = batch_dict['xyz'].contiguous()  # (B, N, 3)
        else:
            xyz = batch_dict['points'][..., :3].contiguous()  # 兜底

        points = batch_dict['points'].contiguous()  # (B, N, C)
        ...
        l0_xyz = xyz.permute(0, 2, 1).contiguous()  # [B, 3, N]
        l0_fea = points.permute(0, 2, 1).contiguous()  # [B, C, N]
        ...
        pred_offset = self.offset_branch(o).permute(0, 2, 1).contiguous()  # (B, N, 3)
        pred_sem = self.sem_branch(s).permute(0, 2, 1).contiguous()  # (B, N, num_classes)
        pred_fea = self.fea_branch(f).permute(0, 2, 1).contiguous()  # (B, N, 64)

        pred_sem_scores = torch.softmax(pred_sem, dim=-1)  # (B, N, num_classes)
        pred_sem_values, pred_sem_indices = torch.max(pred_sem_scores, dim=-1)
        ...
        batch_dict['point_feature'] = pred_fea
        batch_dict['point_pred_score'] = pred_sem_scores
        batch_dict['point_pred_sem'] = pred_sem_indices
    ...
    def loss(self, loss_dict, disp_dict):
        ...
        # loss = sem_loss + off_norm_loss + off_dir_loss
        # loss_dict.update({
        ...
        pred_sem_i = self.train_dict['sem_pred'].reshape(pred_sem.shape[0], -1)
        label_sem_oa = self.train_dict['sem_label'].reshape(pred_sem.shape[0], -1)
        OA = torch.sum((pred_sem_i == label_sem_oa)).item() / label_sem_oa.numel()
    ...
    def get_sem_loss(self, pred, label, weight):
        ...
        for a in range(batch_size):
            pred_ = pred[a, :, :]  # (N, num_classes)
            label_ = label[a].reshape(-1)  # (N,)
